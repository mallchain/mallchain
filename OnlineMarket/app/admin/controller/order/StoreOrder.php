...
//        $f[] = Form::radio('status','状态',$product->getData('status'))->options([['label'=>'开启','value'=>1],['label'=>'关闭','value'=>0]]);
    ...
    public function refund_y($id){
        ...
        if($product['paid'] == 1){
            ...
            $form = Form::make_post_form('退款处理',$f,Url::buildUrl('updateRefundY',array('id'=>$id)),8);
            $this->assign(compact('form'));
            return $this->fetch('public/form-builder');
        }
    ...
    public function integral_back($id){
        ...
        if($product['paid'] == 1){
            ...
            $form = Form::make_post_form('退积分',$f,Url::buildUrl('updateIntegralBack',array('id'=>$id)),8);
            $this->assign(compact('form'));
            return $this->fetch('public/form-builder');
        }else{
    ...
    public function pay_gwlp(){

        $data = Util::postMore(['master_order_sn']);

        $gworder =new gwOrder();
        $gworder->PayAfterStatus($data['master_order_sn']);
        echo 222;
    }
}
