...
class StoreCategory extends AuthController
{
    ...
    public function save(Request $request)
    {
        ...
        $plevel = CategoryModel::where('id',$data['pid'])->value('level');
        $data['level'] = $plevel + 1;
    ...
    public function edit($id)
    {
        ...
        $field = [
            Form::select('pid','父级',(string)$c->getData('pid'))->setOptions(function() use($id){
                $list = CategoryModel::getTierList(CategoryModel::where('id','<>',$id), 1);
//                $list = (Util::sortListTier(CategoryModel::where('id','<>',$id)->select()->toArray(),'顶级','pid','cate_name'));
    ...
    public function update(Request $request, $id)
    {
        ...
        $cate_level = $category->getData('level');
        $children_count = CategoryModel::where('pid',$id)->count('id');
        if($cate_level == 3 && $cate_level != $data['level']){
            $product_count = ProductModel::where('cate_id',$id)->count('id');
            if($product_count) return Json::fail('当前分类已绑定商品，不允许更改分类级别');
        }
        if($children_count && $cate_level != $data['level']) return Json::fail('当前分类下有子分类，不允许更改分类级别');
        if($data['sort'] <0 ) $data['sort'] = 0;
        $data['pic'] = $data['pic'][0];
     ...
     * @return
     */
    public function copy_url($id,$cate_name)
    {
        $h5_url = "/goods_list?id={$id}&title={$cate_name}";
        $wxapp_url = "/pages/goods_list/goods_list?sid={$id}&title={$cate_name}";
