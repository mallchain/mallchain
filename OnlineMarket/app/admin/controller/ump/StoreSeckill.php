...
class StoreSeckill extends AuthController
{
    ...
    public function edit($id)
    {
        ...
        /*$f[] = Form::select('times','秒杀时间段')->setOptions(function(){
            $where = ['gid' => 49,'status' => 1];
            $list = SystemGroupData::getDateValues($where);
            $menus=[];
            foreach ($list as $menu){
                $menus[] = ['value'=>$menu['time'] . '_' . $menu['continued'],'label'=>$menu['time'] . ':00'];//,'disabled'=>$menu['pid']== 0];
            }
            return $menus;
        })->filterable(1);*/
        $f[] = Form::frameImageOne('image','产品主图片(305*305px)',Url::buildUrl('admin/widget.images/index',array('fodder'=>'image')),$product->getData('image'))->icon('image')->width('100%')->height('500px');
