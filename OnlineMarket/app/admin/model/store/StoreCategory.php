...
class StoreCategory extends BaseModel
{
     ...
     */
    public static function getTierList($model = null, $type = 0,$return_level = 2)
    {
        if($model === null) $model = new self();
        if(!$type) return UtilService::sortListTier($model->order('sort desc,id desc')->where('pid', 0)->select()->toArray());
        return UtilService::sortListTier2($model->order('sort desc,id desc')->select()->toArray(),$return_level);
    }
