    ...
    public static function getModelObject($where=[]){
        ...
        if(!empty($where)){
            ...
            if(isset($where['store_name']) && $where['store_name']!=''){
                $model = $model->where('p.store_name|p.keyword|p.id|p.gwlp_goods_id','LIKE',"%$where[store_name]%");
            }
...
//                $model = $model->whereOr('p.cate_id','LIKE',["%$catid%",$catidab]);
            ...
            if(isset($where['price_min']) && $where['price_min']!=''){
                $model = $model->where('p.price','>=',$where['price_min']);
            }
            if(isset($where['price_max']) && $where['price_max']!=''){
                $model = $model->where('p.price','<=',$where['price_max']);
            }
            if(isset($where['profit_min']) && $where['profit_min']!=''){
                $model = $model->where("(p.price - p.cost) / p.price * 100 >= {$where['profit_min']}");
            }
            if(isset($where['profit_max']) && $where['profit_max']!=''){
                $model = $model->where("(p.price - p.cost) / p.price * 100 <= {$where['profit_max']}");
            }
            if(isset($where['price_max']) && $where['price_max']!=''){
                $model = $model->where('p.price','<=',$where['price_max']);
            }
            ...
            if(strlen($where['activity'])>=3){
                switch ($where['activity']) {
                    case 'is_seckill':
                        $model=$model->join('StoreSeckill ss','p.id=ss.product_id','LEFT')
                                ->where([
                                    'ss.is_del' => 0,
                                    'ss.status' => 1
                                ])->where('ss.stop_time','>=',time());
                        break;
                    case 'is_bargain':
                        $model=$model->join('StoreBargain sb','p.id=sb.product_id','LEFT')
                                ->where([
                                    'sb.is_del' => 0,
                                    'sb.status' => 1
                                ])->where('sb.stop_time','>=',time());
                        break;
                    case 'is_combination':
                        $model=$model->join('StoreCombination sc','p.id=sc.product_id','LEFT')
                                ->where([
                                    'sc.is_del' => 0,
                                    'sc.is_show' => 1
                                ])->where('sc.stop_time','>=',time());
                        break;
                    default:
                            $model = $model->where("p.{$where['activity']}",1);
                        break;
                }
            }
        }
    ...
    * 查找该商品是否有秒杀活动
    */
    public static function productIsSeckill($product_id){
        $where = [
            'is_del' => 0,
            'status' => 1,
            'product_id' => $product_id
        ];
        $seckillCount = StoreSeckill::where($where)->where('stop_time','>=',time())->count();
        return $seckillCount > 0 ? 1 : 0;
    }

    /**
    * 查找该商品是否有砍价活动
    */
    public static function productIsBargain($product_id){
        $where = [
            'is_del' => 0,
            'status' => 1,
            'product_id' => $product_id
        ];
        $bargainCount = StoreBargain::where($where)->where('stop_time','>=',time())->count();
        return $bargainCount > 0 ? 1 : 0;
    }

    /**
    * 查找该商品是否有拼团活动
    */
    public static function productIsCombination($product_id){
        $where = [
            'is_del' => 0,
            'is_show' => 1,
            'product_id' => $product_id
        ];
        $combinationCount = StoreCombination::where($where)->where('stop_time','>=',time())->count();
        return $combinationCount > 0 ? 1 : 0;
    }
   ...
   public static function msectime() {
    ...
    /*   echo $model->getLastSql();
        ...
        foreach ($data as &$item){
            ...
            $item['profit'] = round(($item['price'] - $item['cost']) / $item['price'] * 100,2) . '%';//利润率
            $item['is_seckill'] = self::productIsSeckill($item['id']);//是否秒杀
            $item['is_bargain'] = self::productIsBargain($item['id']);//是否砍价
            $item['is_combination'] = self::productIsCombination($item['id']);//是否拼团
        }

        unset($item);
       /* $time=self::msectime();
        echo self::msectime()-$time;
        exit();*/
        if($where['excel']==1){
            $export = [];
     ...
     * @return int
     */
    public static function getEditProfitCount($where)
    {
        if($where['is_all'] == 1)
            $count = self::getModelObject($where)->count();
        else
            $count = self::where('id','in',$where['ids'])->count();
        return $count;
    }
     ...
     * @return array
     */
    public static function updateProfitRate($where){
        if($where['is_all'] == 1){
            $model = self::getModelObject($where)->field(['p.id,p.gwlp_shop_price,p.gwlp_cost_price,p.gwlp_profitrate'])->page((int)$where['page'],(int)$where['limit']);
            $data = ($data=$model->select()) && count($data) ? $data->toArray():[];
        }else{
            $data = self::where('id','in',$where['ids'])->select(); //->cursor() 游标查询
            $data = $data->toArray();
        }
        if(empty($data)) return false;
        $total = 0;
        $integral_ratio = sysConfig('integral_ratio');
        $profit_rate = $where['profit'] / 100;
        foreach ($data as $item){
            $res = false;
            $gwlp_shop_price = $item['gwlp_shop_price'];
            $gwlp_cost_price = $item['gwlp_cost_price'];
            if($gwlp_shop_price && $gwlp_cost_price){
                //if($item['gwlp_profitrate'] > $where['profit']) $profit_rate = $item['gwlp_profitrate'] / 100;
                $price = round($gwlp_cost_price + ($gwlp_shop_price - $gwlp_cost_price) * $profit_rate,2); //根据利润值算出的目标售价
                //$integral = round($price - ($gwlp_cost_price + ($price - $gwlp_cost_price) * $profit_rate)) / $integral_ratio; //根据利润值算出的积分额，之前的算法
                $integral = round($gwlp_shop_price - $price) / $integral_ratio; //根据目标售价算出的积分额，修改后的算法
                if($integral > 0) $res = self::edit(['profit_price' => $price,'use_integral' => $integral],$item['id']);
            }
            //商品规格
            $attrs = ($attrs=StoreProductAttrValue::where('product_id',$item['id'])->select()) && count($attrs) ? $attrs->toArray():[];
            if(!empty($attrs) && !empty($integral)){
                foreach ($attrs as $attr){
                    $gwlp_price = $attr['gwlp_price'];
                    if($gwlp_price && $gwlp_cost_price){
                        $spec_price = $gwlp_cost_price + ($gwlp_price - $gwlp_cost_price) * $profit_rate; //根据利润值算出的规格目标售价
                        $res = StoreProductAttrValue::edit(['profit_price' => $spec_price],$attr['unique'],'unique');
                    }
                }
            }
            if($res !== false) $total++;
