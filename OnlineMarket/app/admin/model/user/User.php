...
//        echo self::getLastSql();die;
    ...
    public static function backOrderBrokerage($orderInfo)
    {
        ...
        //高级别返佣state
        $brokerageRatioS=0;//高级别返佣点数
        $brokeragePriceS=0;//高级别返佣金额
        $res3=true;
        if($userInfo['distributor_value']){
            $brokerageRatioS = (sys_config($userInfo['distributor_value'].'store_brokerage_ratio') ?: 0) / 100;
        }
        //高级别返佣end
        ...
        //高级别返佣state
        if($brokerageRatioS>0){
            $brokeragePriceS = bcmul(bcsub($orderInfo['pay_price'], $cost, 2), $brokerageRatioS, 2);
            $label= C("fei_xiao_rank")[$userInfo['distributor_value']]['label'];
            $mark2 = $userInfo['nickname'] . '成功消费' . floatval($orderInfo['pay_price']) . "元,奖励{$label}推广佣金" . floatval($brokeragePriceS);
            $res3 = UserBill::income("获得{$label}推广佣金", $userInfo['spread_uid'], 'now_money', 'brokerage_top', $brokeragePriceS, $orderInfo['id'], $balance+$brokeragePrice, $mark2);
        }
        //高级别返佣end
        $res2 = self::bcInc($userInfo['spread_uid'], 'now_money', $brokeragePrice+$brokeragePriceS, 'uid');
        $res = $res1 && $res2 && $res3;
        self::checkTrans($res);
    ...
    public static function backOrderBrokerageTwo($orderInfo)
    {
        ...
        $res3=true;
        if($userInfo['distributor_value']){
            $brokerageRatioS = (sys_config($userInfo['distributor_value'].'store_brokerage_two') ?: 0) / 100;
        }
        //高级别返佣end
        ...
        if($brokerageRatioS>0){
            $brokeragePriceS = bcmul(bcsub($orderInfo['pay_price'], $cost, 2), $brokerageRatioS, 2);
            $label= C("fei_xiao_rank")[$userInfo['distributor_value']]['label'];
            $mark2 = '二级推广人' . $userInfo['nickname'] . '成功消费' . floatval($orderInfo['pay_price']) . '元,奖励'.$label.'推广佣金' . floatval($brokeragePriceS);
            $res3 = UserBill::income("获得{$label}推广佣金", $userInfo['spread_uid'], 'now_money', 'brokerage_top', $brokeragePriceS, $orderInfo['id'], $balance+$brokeragePrice, $mark2);
        }
        //高级别返佣end

        $res2 = self::bcInc($userInfoTwo['spread_uid'], 'now_money', $brokeragePrice+$brokeragePriceS, 'uid');
        $res = $res1 && $res2 && $res3;
        self::checkTrans($res);
        return $res;
    }
