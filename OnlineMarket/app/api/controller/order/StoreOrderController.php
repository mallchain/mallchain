...
use app\commen\login\order as gw_order;
...
//        $priceGroup = StoreOrder::getOrderPriceGroup($cartInfo);
    ...
    public function create(Request $request, $key)
    {
        ...
        elseif ($from == 'weixinh5')
            $isChannel = 2;
        $order = StoreOrder::cacheKeyCreateOrder($request->uid(), $key, $addressId, $payType, (int)$useIntegral, $couponId, $mark, $combinationId, $pinkId, $seckill_id, $bargainId, false, $isChannel,$shipping_type,$real_name,$phone,true);
        ...
        if($result === NULL){
            ...
            $result=[
                ...
                'result'=>[
                    ...
                    'type'=> $order['delivery_name'],
                    'list'=>$obj->GetLogisticsInfo($order['master_order_sn']),
                ],
            ];
            $cacheTime = 1800;
    ...
    public function refund_verify(Request $request)
    {
        ...
        if ($res){
            // TODO 广物供应链订单先请求广物供应链取消订单接口，若不能直接取消则申请广物供应链售后
            $gw_order = new gw_order();
            $gw_order->refundGWLPOrder($uni, $request->uid(),$data['text'], $data['refund_reason_wap_explain']);
