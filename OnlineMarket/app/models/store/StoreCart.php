...
class StoreCart extends BaseModel
{
    ...
    {
        $productInfoField = 'id,image,price,ot_price,vip_price,postage,give_integral,sales,stock,store_name,unit_name,is_show,is_del,is_postage,cost,use_integral,gwlp_goods_id';
        ...
        foreach ($list as $k=>$cart){
            ...
            //广物库存查询
            $gwlp_goods_id=0;
            if(isset($product['gwlp_goods_id'])) $gwlp_goods_id=$product['gwlp_goods_id'];

           $is_stock_ok= $gwlp_obj->cache_stock($cart['product_id'],$gwlp_goods_id,$uid,$address);
            //广物库存查询
            ...
            //商品删除或无库存
            }else if(!$product['is_show'] || $product['is_del'] || !$product['stock'] || !$is_stock_ok){
                $invalid[] = $cart;
            //商品属性不对应
