...
//        $totalPrice = bcadd($totalPrice,$storePostage,2);
    ...
    public static function cacheKeyCreateOrder($uid, $key, $addressId, $payType, $useIntegral = false, $couponId = 0, $mark = '', $combinationId = 0, $pinkId = 0, $seckill_id = 0, $bargain_id = 0, $test = false, $isChannel = 0, $shipping_type = 1, $real_name = '', $phone = '',$is_gwlp_order=false)
    {
        self::beginTrans();
        try {
            ...
            $is_synchronization_order=ConfigModel::getConfigValue('is_synchronization_order');
            $order_id= self::getNewOrderId();
            if($is_synchronization_order==C('is_synchronization_order')){
                //广物下单 start
                if($is_gwlp_order){
                    $gworder =new gwOrder();
                  /*  var_dump($cartGroup['cartInfo']);
                    exit();*/
                    $gw= $gworder->SubmitOrder($cartGroup['cartInfo'],$addressInfo,$order_id);
                    if(!$gw['succeed']){
                         self::clearCacheOrderInfo($uid, $key);
                        return self::setErrorInfo($gw['msg'], true);
                    }
                    $master_order_sn= $gw['master_order_sn'];
                }
                //广物下单end
            }
            ...
            $totalNum = 0;
            $gainIntegral = 0;
            $availableIntegral = 0; //最多可使用的积分总和
            foreach ($cartInfo as $cart) {
                $cartIds[] = $cart['id'];
                ...
                $cartInfoAvailableIntegral = !empty($cart['productInfo']['use_integral']) ? bcmul($cart['cart_num'], $cart['productInfo']['use_integral'], 2) : 0;
                $gainIntegral = bcadd($gainIntegral, $cartInfoGainIntegral, 2);
                $availableIntegral = bcadd($availableIntegral, $cartInfoAvailableIntegral, 2);
            }
            ...
            $res2 = true;
            $SurplusIntegral = 0;
            if ($useIntegral && $userInfo['integral'] > 0 && $availableIntegral > 0) {
                if($userInfo['integral'] < $availableIntegral) $usedIntegral = $userInfo['integral'];
                else $usedIntegral = $availableIntegral;
                $deductionPrice = (float)bcmul($usedIntegral, $other['integralRatio'], 2);
                ...
                if ($deductionPrice < $payPrice) {
                    ...
                    //$SurplusIntegral = 0;
                    $res2 = false !== User::edit(['integral' => $SurplusIntegral], $userInfo['uid'], 'uid');
                } else {
    ...
    public static function cancelOrder($order_id, $uid)
    {
        ...
        //广物取消订单 stata
        $gworder = new gwOrder();
        if(!empty($order['master_order_sn'])){$gworder->CancelOrder($order['master_order_sn']);}
        //文物取消订单 end
        self::beginTrans();
        try {
...
//        if($orderInfo['pay_type'] != 'yue') return self::setErrorInfo('该订单不能使用余额支付!');
    ...
    public static function paySuccess($orderId, $paytype = 'weixin', $formId = '')
    {
        ...
        if($is_synchronization_order==C('is_synchronization_order')) {
            //广物修改支付状态 start
            $gworder = new gwOrder();
            $gworder->PayAfterStatus($order['master_order_sn']);
            //广物修改支付状态 end
        }
        //支付成功后
