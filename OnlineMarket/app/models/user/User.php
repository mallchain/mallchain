...
//            'is_promoter'=>$spread_uid || $storeBrokerageStatu != 1 ? 1: 0,
    ...
    public static function backOrderBrokerage($orderInfo,bool $open = true)
    {
        ...
        //TODO 添加用户余额
        $res2 = self::bcInc($userInfo['spread_uid'], 'brokerage_price', $brokeragePrice+$brokeragePriceS, 'uid');
        //TODO 一级返佣成功 跳转二级返佣
        $res = $res1 && $res2 && $res3 && self::backOrderBrokerageTwo($orderInfo,$open);
...
//        if($res) return self::backOrderBrokerageTwo($orderInfo);
    ...
    public static function backOrderBrokerageTwo($orderInfo,bool $open = true)
    {
        ...
        //TODO 添加用户余额
        $res2 = self::bcInc($userInfoTwo['spread_uid'], 'brokerage_price', $brokeragePrice+$brokeragePriceS, 'uid');
        $res = $res1 && $res2  && $res3;
        $open && self::checkTrans($res);
        return $res;
    }
