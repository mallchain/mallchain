    ...
    public static function make_post_form($title, array $field, $url, $jscallback = 2)
    {
        ...
        switch ($jscallback) {
            ...
            case 7:
                ...
                break;
            case 8:
                $js = '$r.btn.disabled(false);setTimeout(function(){parent.layer.close(parent.layer.getFrameIndex(window.name));},2000);';//父级不刷新 不能再提交 关闭弹窗
                break;
            case 9:
                $js = 'parent.$(".J_iframe:visible")[0].contentWindow.reloadCurrent();setTimeout(function(){parent.layer.close(parent.layer.getFrameIndex(window.name));},2000);';//父级刷新列表当前页 关闭弹窗
                break;
            default:
                $js = $jscallback;
                break;
