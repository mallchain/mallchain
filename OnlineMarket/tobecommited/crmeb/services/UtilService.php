...
//解密时会通过这个密匙验证数据完整性
     ...
     * @return array
     */
    public static function sortListTier2($data,$return_level, $pid = 0, $field = 'pid', $pk = 'id', $html = '|-----', $level = 1, $clear = true)
    {
        static $list = [];
        if ($clear) $list = [];
        foreach ($data as $k => $res) {
            if ($res[$field] == $pid && $level <= $return_level) {
                $res['html'] = $level > 1 ? str_repeat(html_entity_decode('&emsp;'), $level) . $html : $html;
                $list[] = $res;
                unset($data[$k]);
                self::sortListTier2($data,$return_level, $res[$pk], $field, $pk, $html, $level + 1, false);
            }
        }
        return $list;
    }
...
//                        $tip = "校验位错误";
     ...
     * @return string
     */
    public static function getIntegralMoney($integral)
    {
        $integral_ratio = sysConfig('integral_ratio'); //积分抵扣换算比例
        return bcmul($integral,$integral_ratio,2);
    }
}